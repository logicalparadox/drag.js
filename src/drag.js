      ...
      var moveHandler = function (e2) {
        ...
        if (self._container) {
          ...
          if (newX < 0) newX = 0;
            ...
            maxX = (drag.value(c, 'padding-left') + 
                    drag.value(c, 'width') + 
                    drag.value(c, 'padding-right')) - 
                   (drag.value(self.el, 'margin-left') + 
                    drag.value(self.el, 'border-left-width') + 
                    drag.value(self.el, 'padding-left') + 
                    drag.value(self.el, 'width') + 
                    drag.value(self.el, 'padding-right') + 
                    drag.value(self.el, 'border-right-width') + 
                    drag.value(self.el, 'margin-right'));
            if (newX > maxX) newX = maxX;
          }
          if (newY < 0) newY = 0;
          else if (newY >= 0) {
            maxY = (drag.value(c, 'padding-top') + 
                    drag.value(c, 'height') + 
                    drag.value(c, 'padding-bottom')) - 
                   (drag.value(self.el, 'margin-top') + 
                    drag.value(self.el, 'border-top-width') + 
                    drag.value(self.el, 'padding-top') + 
                    drag.value(self.el, 'height') + 
                    drag.value(self.el, 'padding-bottom') + 
                    drag.value(self.el, 'border-bottom-width') + 
                    drag.value(self.el, 'margin-bottom'));
